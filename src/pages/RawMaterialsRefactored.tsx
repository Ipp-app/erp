...
export default function RawMaterialsRefactored() {
  const {
    data: materials,
    loading,
    showForm,
  ...
  const columns = [
    ...
    { 
      ...
      render: (value: number, item: RawMaterial) => {
        if (value <= (item.minimum_stock * 0.5)) return <span className="text-red-600 font-bold">Critical</span>;
        if (value <= item.minimum_stock) return <span className="text-orange-600 font-bold">Low</span>;
        if (value >= item.maximum_stock) return <span className="text-blue-600 font-bold">Overstock</span>;
    ...
    { 
      key: 'image_url' as keyof RawMaterial, 
      label: 'Image',
      render: (value: string) => value ? 
        <img src={value} alt="material" className="w-12 h-12 object-cover" /> : '-'
    }
  ];
